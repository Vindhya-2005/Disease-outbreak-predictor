...
print(f"CatBoost Model trained successfully! (Accuracy: {accuracy:.4f})")
...
def predict_outbreaks_for_disease(year, month, disease_name):
    ...
    probs = cat_model.predict_proba(future_X_scaled)
    disease_index = np.where(cat_model.classes_ == le_disease.transform([disease_name])[0])[0][0]
