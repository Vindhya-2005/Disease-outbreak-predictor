...
# ------------------------------
# ✅ SVM MODEL (replaces GradientBoostingClassifier)
# ------------------------------
...
print(f"SVM Model trained successfully! (Accuracy: {accuracy:.4f})")
...
def predict_outbreaks_for_disease(year, month, disease_name):
    ...
    probs = svm_model.predict_proba(future_X_scaled)
    disease_index = np.where(svm_model.classes_ == le_disease.transform([disease_name])[0])[0][0]
