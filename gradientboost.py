...
print(f"Gradient Boosting Model trained successfully! (Accuracy: {accuracy:.4f})")
...
def predict_outbreaks_for_disease(year, month, disease_name):
    ...
    probs = gb_model.predict_proba(future_X_scaled)
    disease_index = np.where(gb_model.classes_ == le_disease.transform([disease_name])[0])[0][0]
...
import markdown  # Add at the top of your file
...
def chat():
    ...
    # Return the HTML reply to your frontend
